...
Moeda <-  ts(log(aelf$aelf), start = 2018, frequency = 365)


#Se não for estacionária, diferenciar a série

IntOrdem1 <- diff(log(aelf$aelf))
IntegradaOrdem1 <- ts(IntOrdem1, start = 2018, frequency = 365)

plot(IntegradaOrdem1, type="l", main="Primeira Diferança dos Logs do Aelf - LogReturn", ylab="Log Preço", xlab="Data", col="Blue")
...
for (i in 1:21) {                 #Loop para os AR: ARIMA(i,0,0)
  ...
  est1[i,2] <- AIC(arima(IntegradaOrdem1,  order = c(i,0,0)))  #Coluna com valores AIC
  est1[i,3] <- BIC(arima(IntegradaOrdem1,  order = c(i,0,0)))  #Coluna com valores BIC
}
...
GARCH202 <- garch(IntegradaOrdem1,c(0,0),trace=F)
...
estimacoes_garch <- list(GARCH202)

AIC_Garch <- sapply(estimacoes_garch, AIC)      #Cria Coluna com resultados AIC

Modelos_Garch <- c("GARCH202")                                            #cria coluna com nome dos modelos
